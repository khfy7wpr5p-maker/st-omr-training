...
from st_omr_training.external_benchmark_harness import (
    ...
    TargetFormat,
    admitted_dataset_manifest_sha256,
    benchmark_spec,
    create_admission,
...
def _replace_catalog_record(old, new):
    return tuple(new if item is old else item for item in EXTERNAL_DATASET_CANDIDATES)


def _pinned_commercial(record, artifact_sha256: str):
    return replace(
        record,
        license_id="TEST-COMMERCIAL-CLEAN",
        redistribution_allowed=True,
        commercial_use_allowed=True,
        training_allowed=True,
        evaluation_allowed=True,
        derivative_restrictions="test fixture",
...
class ExternalBenchmarkHarnessTests(unittest.TestCase):
    ...
    def test_admitted_dataset_hash_binds_artifact_bytes(self) -> None:
        spec = benchmark_spec(BenchmarkDatasetKind.OLIMPIC_SCANNED)
        rows = _rows()
        self.assertNotEqual(
            admitted_dataset_manifest_sha256(rows, spec, SHA_C),
            admitted_dataset_manifest_sha256(rows, spec, SHA_D),
        )
        first = create_admission(
            spec=spec,
            registry_records=EXTERNAL_DATASET_CANDIDATES,
            rows=rows,
            data_artifact_sha256=SHA_C,
            admission_mode=AdmissionMode.RESEARCH_OVERRIDE,
            research_override_reference="override-a",
        )
        second = create_admission(
            spec=spec,
            registry_records=EXTERNAL_DATASET_CANDIDATES,
            rows=rows,
            data_artifact_sha256=SHA_D,
            admission_mode=AdmissionMode.RESEARCH_OVERRIDE,
            research_override_reference="override-b",
        )
        self.assertNotEqual(first.benchmark_identity(), second.benchmark_identity())
    ...
    def test_manifest_canonicalizes_equivalent_paths_before_duplicate_detection(self) -> None:
        spec = benchmark_spec(BenchmarkDatasetKind.OLIMPIC_SCANNED)
        first = BenchmarkManifestRow(
            sample_id=SHA_A,
            family_id="score-1",
            split="validation",
            image_relpath="images/a.png",
        ...
        self.assertEqual(second.image_relpath, "images/a.png")
        with self.assertRaises(ExternalBenchmarkHarnessError):
            validate_manifest_rows((first, second), spec)
    ...
    def test_strict_research_only_record_never_becomes_commercial_evidence(self) -> None:
        ...
        catalog = _replace_catalog_record(record, pinned)
    ...
    def test_strict_commercial_clean_record_can_be_commercial_evidence(self) -> None:
        ...
        record = matching_registry_record(spec, EXTERNAL_DATASET_CANDIDATES)
        pinned = _pinned_commercial(record, SHA_C)
        catalog = _replace_catalog_record(record, pinned)
        admission = create_admission(
            spec=spec,
            registry_records=catalog,
    ...
    def test_grandstaff_strict_admission_requires_original_dataset_component(self) -> None:
        spec = benchmark_spec(BenchmarkDatasetKind.GRANDSTAFF_LMX)
        annotation = matching_registry_record(spec, EXTERNAL_DATASET_CANDIDATES)
        original = next(
            item
            for item in EXTERNAL_DATASET_CANDIDATES
            if item.dataset_name == "GrandStaff"
            and item.dataset_component == "original pianoform dataset"
        )
        pinned_annotation = _pinned_commercial(annotation, SHA_A)
        catalog = tuple(
            pinned_annotation if item is annotation else item
            for item in EXTERNAL_DATASET_CANDIDATES
        )
        rows = (
            BenchmarkManifestRow(
                sample_id=SHA_A,
                family_id="grandstaff-score",
                split="test",
                image_relpath="images/system.png",
                target_relpath="targets/system.lmx",
                system_id="system",
            ),
        )
        with self.assertRaises(ExternalBenchmarkHarnessError):
        ...
        pinned_original = _pinned_commercial(original, SHA_B)
        fully_pinned = tuple(
            pinned_annotation
            if item is annotation
            else pinned_original
            if item is original
            else item
            for item in EXTERNAL_DATASET_CANDIDATES
        )
        admission = create_admission(
            spec=spec,
            registry_records=fully_pinned,
            rows=rows,
            data_artifact_sha256=SHA_C,
            admission_mode=AdmissionMode.STRICT_REGISTRY,
        )
        self.assertEqual(len(admission.registry_record_sha256s), 2)
        self.assertTrue(admission.commercial_evidence_eligible)
