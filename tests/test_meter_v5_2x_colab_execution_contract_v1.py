...
class TestMeterV5_2XColabExecutionContractV1(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.path = (
            Path(__file__).resolve().parents[1]
            / "notebooks"
            / "st_omr_meter_v5_2x_minimum_functional_logit_drift_audit_colab.ipynb"
        )
    ...
    def test_notebook_is_single_run_and_exact_ci_green_head_pinned(self):
        ...
        self.assertIn(
            'EXPECTED_HEAD = "c276517e07ee129d80b53bb906ead06c3094f0af"',
            self.source,
        )
        self.assertIn("EXPECTED_CI_RUN_ID = 32701812707", self.source)
        self.assertIn(
    ...
    def test_notebook_runs_only_the_v5_2x_diagnostic_once(self):
        self.assertEqual(
            self.source.count("run_minimum_functional_logit_drift_audit_v1("),
            1,
        )
        forbidden = (
    ...
    def test_notebook_fails_closed_and_verifies_aggregate_minimax_evidence(self):
        required = (
            ...
            "minimum_max_absolute_historical_logit_drift",
            ...
            "weight_norm_minimized",
            "witness_values_emitted",
            "witness_persisted",
            "v5_2x_execution_envelope_",
        )
