...
D6 is a data gate, not a model-training stage. It reuses the accepted frozen
Synthetic Curriculum v1 and D5 geometry machinery to create small canonical
JSON labels for TRAIN and VALIDATION only. Existing PNGs are referenced by
SHA-256 rather than copied. TEST rows are skipped immediately after reading the
split field, before any specialist artifact path, image hash or label is derived.
"""
...
MAX_TARGET_BYTES: Final[int] = 4 * 1024 * 1024
...
_GEOMETRY_KEYS = {
    ...
    "staff_instances",
    "measures",
    "geometry_transform_fingerprint",
}
_STAFF_KEYS = {
    "staff_instance_id",
    "system_id",
...
_SYSTEM_KEYS = {"system_id", "system_bbox", "staff_instance_id", "measure_numbers"}
_MEASURE_KEYS = {
    "measure_id",
    "measure_number",
    "system_id",
...
class Stage7D6DerivativeError(RuntimeError):
    """Raised when D6 cannot derive or independently validate safe sidecars."""


@dataclass(frozen=True, slots=True)
class _ManifestDegradedPage:
    """Manifest-backed Stage-4 view sufficient for the D5 coordinate mapper."""
...
def _canonical_json(payload: object) -> bytes:
    try:
        ...
        raise Stage7D6DerivativeError(
            "payload is not canonical JSON serializable"
        ) from exc


def _persisted_json_shape(payload: object) -> object:
    """Return exactly the list/dict/scalar shape that canonical JSON persists.

    ``dataclasses.asdict`` intentionally preserves tuple containers. JSON arrays
    are persisted as lists when reloaded, so validation must run on this shape,
    not on the transient dataclass representation.
    """

    try:
        return json.loads(_canonical_json(payload).decode("ascii"))
    except (UnicodeError, json.JSONDecodeError) as exc:  # pragma: no cover
        raise Stage7D6DerivativeError("canonical JSON round-trip failed") from exc
...
def _load_canonical_json(
    ...
    try:
        ...
        raise Stage7D6DerivativeError(
            f"{label} is not valid ASCII JSON"
        ) from exc
...
def _development_rows(samples: object) -> tuple[Mapping[str, object], ...]:
    """Expose only TRAIN/VALIDATION rows after the explicit D6 split seal.

    TEST is skipped after reading *only* ``split``. Regression tests use hostile
    Mapping objects that raise if any other TEST field is touched.
    """
...
def _verify_d1_receipt(receipt: SyntheticCurriculumCorpusReceipt) -> None:
    ...
    for key, value in expected.items():
        if getattr(receipt, key) != value:
            _fail(
                f"D1 receipt {key} does not match frozen Synthetic Curriculum v1"
            )
    if receipt.sample_split_counts != {
        "test": 153,
        **EXPECTED_DEVELOPMENT_SAMPLE_COUNTS,
    }:
        _fail("D1 receipt sample split counts mismatch")
    if receipt.family_split_counts != {
        "test": 51,
...
def _config_from_row(row: Mapping[str, object]) -> DegradationConfig:
    ...
    try:
        ...
        raise Stage7D6DerivativeError(
            "development degradation_config is invalid"
        ) from exc
...
def _manifest_page_view(
    row: Mapping[str, object], config: DegradationConfig
) -> _ManifestDegradedPage:
    return _ManifestDegradedPage(
        ...
        clean_height=_require_positive_int(
            "clean_height", row.get("clean_height")
        ),
...
def _page_by_number(
    pages: Sequence[object], page_number: int, label: str
) -> object:
    matches = [
        page
        for page in pages
...
def _validate_point_dict(point: object, width: int, height: int) -> None:
    ...
    epsilon = 1e-5
    if not -epsilon <= x <= width + epsilon:
        _fail("geometry point x lies outside final PNG bounds")
    if not -epsilon <= y <= height + epsilon:
        _fail("geometry point y lies outside final PNG bounds")
...
def _validate_geometry_payload(
    ...
    *,
    ...
    if not isinstance(geometry, Mapping) or set(geometry) != _GEOMETRY_KEYS:
        _fail("geometry payload shape mismatch")
    if geometry.get("page_number") != page_number:
        _fail("geometry page_number lineage mismatch")
    ...
    view_box = geometry.get("view_box")
    if not isinstance(view_box, list) or len(view_box) != 4:
        _fail("geometry view_box must be a four-value JSON array")
    ...
    staff_system_refs: set[str] = set()
    for staff in staffs:
        if not isinstance(staff, Mapping) or set(staff) != _STAFF_KEYS:
            _fail("staff instance shape mismatch")
        staff_id = _require_id(
            "staff_instance_id", staff.get("staff_instance_id")
        )
        if staff_id in staff_ids:
            _fail("duplicate staff_instance_id")
        staff_ids.add(staff_id)
        staff_system_refs.add(_require_id("staff.system_id", staff.get("system_id")))
        ...
        if _finite_number(staff.get("staff_spacing")) <= 0:
            _fail("staff_spacing must be positive")

    system_ids: set[str] = set()
    covered_numbers: set[int] = set()
    for system in systems:
        if not isinstance(system, Mapping) or set(system) != _SYSTEM_KEYS:
            _fail("system shape mismatch")
        ...
        for number in numbers:
            ...
            if number in covered_numbers:
                _fail("measure number is covered by multiple systems")
            covered_numbers.add(number)

    if staff_system_refs != system_ids:
        _fail("staff/system cross references do not match exactly")

    measure_numbers: set[int] = set()
    for measure in measures:
        if not isinstance(measure, Mapping) or set(measure) != _MEASURE_KEYS:
            _fail("measure shape mismatch")
        _require_id("measure_id", measure.get("measure_id"))
    ...
    if covered_numbers != measure_numbers:
...
def _development_row_metadata(row: Mapping[str, object]) -> dict[str, object]:
    ...
    if row.get("mode") not in {None, "L"}:
        _fail("development image mode must be L")
    if row.get("image_format") not in {None, "png"}:
        _fail("development image format must be png")
    return {
        ...
        "source_svg_sha256": _require_sha(
            "source_svg_sha256", row.get("source_svg_sha256")
        ),
        "renderer_config_fingerprint": _require_sha(
            "renderer_config_fingerprint", row.get("renderer_config_fingerprint")
        ),
...
def _label_payload(
    row: Mapping[str, object], mapped: PageGeometry
) -> dict[str, object]:
    meta = _development_row_metadata(row)
    config_fingerprint = str(meta["degradation_config_fingerprint"])
    if mapped.geometry_transform_fingerprint is None:
        _fail("mapped geometry is missing transform fingerprint")

    # Validate exactly what will be persisted. This converts dataclass tuples to
    # JSON arrays before shape validation and closes the CI #161 finding.
    geometry = _persisted_json_shape(asdict(mapped))
    _validate_geometry_payload(
        geometry,
        width=int(meta["width"]),
        height=int(meta["height"]),
        source_musicxml_sha256=str(meta["source_musicxml_sha256"]),
    ...
    return {
        ...
        "lineage": {
            "source_musicxml_sha256": meta["source_musicxml_sha256"],
            "source_svg_sha256": meta["source_svg_sha256"],
            "renderer_config_fingerprint": meta[
                "renderer_config_fingerprint"
            ],
            "degradation_config_fingerprint": config_fingerprint,
...
def _build_into(
    ...
    for row in rows:
        ...
        if renderer_fingerprint != frozen_renderer_fingerprint:
            _fail(
                "development renderer fingerprint differs from frozen D5 renderer"
            )
        ...
        if prior_lineage != lineage:
            _fail(
                "family derivatives disagree on MusicXML/SVG/renderer lineage"
            )

        cached = family_cache.get(family_id)
        if cached is None:
            target_path = (
                corpus_root / "targets" / f"{target_sha}.musicxml"
            )
            target_bytes = _read_bounded(
                target_path, MAX_TARGET_BYTES, "development MusicXML"
            )
            ...
            geometry_render = render_musicxml_geometry_svg(
                target_bytes, renderer_config
            )
            ...
            if (
                geometry_render.base_renderer_config_fingerprint
                != renderer_fingerprint
            ):
                _fail("geometry renderer configuration changed")

            geometry_pages = extract_staff_structure_geometry(
                geometry_render, target_bytes
            )
        ...
        page_number = int(meta["page_number"])
        base_page = _page_by_number(
            base_render.pages, page_number, "base render"
        )
        if getattr(base_page, "sha256", None) != source_svg_sha:
            _fail(
                "pinned Verovio source SVG hash does not match frozen corpus lineage"
            )
        svg_geometry = _page_by_number(
            geometry_pages, page_number, "geometry render"
        )
        if not isinstance(svg_geometry, PageGeometry):
            _fail("geometry page has unexpected type")
    ...
    build_payload = {
        ...
        "family_count": len(
            {str(record["family_id"]) for record in records}
        ),
        ...
        "layout": {
            ...
            "source_images": (
                "external frozen corpus images/<png_sha256>.png"
            ),
        },
    }
...
def _validate_label_against_source(
    ...
    if dict(lineage) != {
        ...
        "renderer_config_fingerprint": source[
            "renderer_config_fingerprint"
        ],
        "degradation_config_fingerprint": source[
            "degradation_config_fingerprint"
        ],
        "d5_geometry_version": STAGE7D5_GEOMETRY_VERSION,
        "d5_transform_version": STAGE7D5_TRANSFORM_VERSION,
...
def verify_stage7d6_derivatives(
    ...
    """Independently validate persisted D6 artifacts against frozen manifest."""

    if not isinstance(corpus_root, (str, Path)) or not isinstance(
        derivative_root, (str, Path)
    ):
        raise TypeError(
            "corpus_root and derivative_root must be str or pathlib.Path"
        )
    ...
    manifest_sha = sha256(manifest_bytes).hexdigest()
    checksum = _read_bounded(
        root / "manifest.sha256", 256, "D6 manifest.sha256"
    )
    ...
    if records != sorted(
        records,
        key=lambda item: (
            str(item.get("sample_id")) if isinstance(item, Mapping) else ""
        ),
    ):
    ...
    for index, record in enumerate(records):
        ...
        if source is None:
            _fail(
                "D6 record does not correspond to a development source sample"
            )
        ...
        binding_rows.append(
            f"{sample_id}:{image_sha}:{label_sha}:{len(raw_label)}"
        )
    ...
    build, _build_bytes = _load_canonical_json(
        root / "build.json", MAX_BUILD_BYTES, "D6 build.json"
    )
    expected_build = {
        "schema_version": STAGE7D6_BUILD_SCHEMA,
        "stage7d6_version": STAGE7D6_VERSION,
    ...
    if build != expected_build:
        _fail(
            "D6 build.json does not match independently derived expectations"
        )
...
def build_stage7d6_derivatives(
    ...
    """Build and independently gate the frozen development derivative set."""

    if not all(
        isinstance(value, (str, Path))
        for value in (corpus_root, transport_archive, output_root)
    ):
        raise TypeError(
            "corpus_root, transport_archive and output_root must be str or pathlib.Path"
        )
    source_root = Path(corpus_root)
    ...
    try:
        ...
        raise Stage7D6DerivativeError(
            "unable to resolve D6 source/output paths"
        ) from exc
    ...
    # D1 may hash TEST artifacts only as frozen-corpus integrity evidence. No
    # TEST paths/labels are returned into D6 specialist development below.
    d1_receipt = verify_stage7d_corpus(source_root, transport_archive)
    _verify_d1_receipt(d1_receipt)
    _source_manifest, rows = _verified_source_manifest(source_root)
...
def canonical_stage7d6_receipt(receipt: Stage7D6DerivativeReceipt) -> bytes:
    ...
    return _canonical_json(
        {"schema_version": STAGE7D6_RECEIPT_SCHEMA, **asdict(receipt)}
    ) + b"\n"
