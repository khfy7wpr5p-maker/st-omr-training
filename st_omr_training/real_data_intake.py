...
class RealDataByteReceipt:
    ...
    def __post_init__(self) -> None:
        if self.intake_version != STAGE8_INTAKE_VERSION:
        ...
        if (
            not isinstance(self.perceptual_hash64, str)
            or len(self.perceptual_hash64) != 16
            or any(ch not in "0123456789abcdef" for ch in self.perceptual_hash64)
        ):
        ...
        if self.image_width * self.image_height > MAX_TRAINING_IMAGE_PIXELS:
...
def _validate_source_document_bytes(data: object) -> bytes:
    # Source documents remain format-agnostic at this gate. The exact bytes are
    # bounded and hash-bound; only the derived training PNG and MusicXML are
    # format/semantic inputs to later ST-OMR training.
...
def validate_quarantined_sample_bytes(
    ...
    *,
    ...
    if record.split is RealDataSplit.TEST:
        # Fail before touching any caller-provided held-out bytes.
    ...
    assert isinstance(training_image_png_bytes, bytes)
...
def validate_byte_receipt(sample: object, receipt: object) -> None:
    """Bind a Stage 8-1 receipt to the exact metadata record it claims to verify."""
...
def find_near_duplicate_candidates(receipts: object) -> tuple[NearDuplicateCandidate, ...]:
    """Flag dHash64 near-duplicates for family/leakage review."""

    if not isinstance(receipts, tuple):
        raise RealDataIntakeError("receipts must be an immutable tuple")
    for receipt in receipts:
        if not isinstance(receipt, RealDataByteReceipt):
            raise RealDataIntakeError("receipts must contain RealDataByteReceipt values")
    ...
    ordered = sorted(receipts, key=lambda item: item.sample_id)
...
def validate_stage8_development_handoff(
    ...
    The Stage 8-0 manifest remains the rights/pairing/leakage authority. Stage 8-1
    additionally requires exactly one matching byte receipt per admitted sample.
    Perceptual candidates are allowed only when both records are already grouped
    into the same family (and therefore, by the manifest gate, the same split).
    """
