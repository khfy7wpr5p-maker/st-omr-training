"""Durable shadow execution for Meter V3-A2 positive-class margin recovery.

This run is stacked on V3-A1. The exact D11 model and bbox outputs remain
fully frozen; only the unchanged classification adapter may update. The sole
new training term is the frozen REAL-TRAIN positive-class pairwise margin.
"""
...
from .meter_real_domain_adaptation_v3_a2 import (
    FROZEN_ADAPTATION_CONFIG_V3_A2,
    METER_REAL_DOMAIN_ADAPTATION_V3_A2,
    MeterRealDomainAdaptationConfigV3A2,
    MeterRealDomainAdaptationV3A2Error,
    adaptation_acceptance_v3_a2,
    build_meter_classification_adapter_v3_a2,
    meter_real_domain_adaptation_fingerprint_v3_a2,
    train_batch_v3_a2,
)
from .meter_teacher_gold_admission_v1 import METER_CLASSES, verify_meter_teacher_gold_bundle_v1


METRICS_SCHEMA_V3_A2: Final[str] = "st-omr-meter-real-domain-adaptation-metrics-v3-a2"
VERIFICATION_SCHEMA_V3_A2: Final[str] = "st-omr-meter-real-domain-adaptation-verification-v3-a2"
CHECKPOINT_ROLE_V3_A2: Final[str] = "meter-real-domain-shadow-candidate-v3-a2"
RESUME_ROLE_V3_A2: Final[str] = "meter-real-domain-adaptation-resume-v3-a2"


def _fail(message: str) -> None:
    raise MeterRealDomainAdaptationV3A2Error(message)


def run_meter_real_domain_adaptation_v3_a2(
    *,
    ...
    if not isinstance(config, MeterRealDomainAdaptationConfigV3A2):
        raise TypeError("config must be MeterRealDomainAdaptationConfigV3A2")
    if config != FROZEN_ADAPTATION_CONFIG_V3_A2:
        _fail("Meter V3-A2 requires the frozen configuration")
    ...
    try:
        ...
        raise MeterRealDomainAdaptationV3A2Error(
            "torch and the pinned training runtime are required for Meter V3-A2"
        ) from exc
    ...
    profile = meter_real_domain_adaptation_fingerprint_v3_a2(
    ...
    model = build_meter_classification_adapter_v3_a2(base_model)
    trainable = [parameter for parameter in model.parameters() if parameter.requires_grad]
    if not trainable or any(parameter.requires_grad for parameter in model.base.parameters()):
        _fail("V3-A2 must train only the classification adapter")
    if hasattr(model, "bbox_delta_head"):
        _fail("V3-A2 must not expose a trainable bbox head")
    ...
    if resume_path.exists():
        if resume_path.is_symlink() or not resume_path.is_file():
            _fail("V3-A2 resume state must be a regular file")
        try:
            snapshot = torch.load(resume_path, map_location="cpu", weights_only=True)
        except Exception as exc:
            raise MeterRealDomainAdaptationV3A2Error("V3-A2 resume state cannot be loaded safely") from exc
        if not isinstance(snapshot, Mapping):
            _fail("V3-A2 resume state must be a mapping")
        checks = {
            "role": RESUME_ROLE_V3_A2,
            "adaptation_version": METER_REAL_DOMAIN_ADAPTATION_V3_A2,
        ...
        for name, expected in checks.items():
            if snapshot.get(name) != expected:
                _fail(f"V3-A2 resume state {name} mismatch")
        try:
            ...
            if not 1 <= completed_epoch <= config.epochs:
                _fail("V3-A2 resume epoch is outside the frozen run")
            ...
            if not isinstance(decision, Mapping):
                _fail("V3-A2 resume best decision is malformed")
            reasons = decision.get("reasons")
            if not isinstance(reasons, Sequence) or isinstance(reasons, (str, bytes, bytearray)):
                _fail("V3-A2 resume reasons are malformed")
            best_decision = AdaptationGateDecisionV2(
                bool(decision.get("accepted")), tuple(str(value) for value in reasons)
            )
            best_real = _evaluation_from_payload_v1(snapshot["best_real"], name="V3-A2 resume best real")
            best_synthetic = _evaluation_from_payload_v1(
                snapshot["best_synthetic"], name="V3-A2 resume best synthetic"
            )
            history = list(snapshot["history"])
        except MeterRealDomainAdaptationV3A2Error:
            raise
        except (KeyError, TypeError, ValueError, RuntimeError) as exc:
            raise MeterRealDomainAdaptationV3A2Error("V3-A2 resume state is malformed") from exc
        if progress is not None:
            progress(
                "resume_loaded",
    ...
    if len(history) != completed_epoch or optimizer_steps != completed_epoch * batches_per_epoch:
        _fail("V3-A2 resume history/optimizer counters are inconsistent")
    ...
    for epoch in range(completed_epoch + 1, config.epochs + 1):
        ...
        if synthetic_metrics.positive_localization_f1_2px != baseline_synthetic.positive_localization_f1_2px:
            _fail("V3-A2 changed synthetic localization despite exact frozen bbox output")
        decision = adaptation_acceptance_v3_a2(
    ...
    if (ending_sha, ending_origin) != (repository_sha, repository_origin):
        _fail("repository identity changed during Meter V3-A2")
    if best_state is not None:
        model.load_state_dict(best_state, strict=True)
    assert_model_finite(model)
    if model_state_sha256(model.base) != base_state_sha:
        _fail("V3-A2 mutated the fully frozen D11 model")
    ...
    if best_state is not None:
        ...
        if len(set(replay_fingerprints)) != 1:
            _fail("V3-A2 candidate inference is not deterministic 10/10")
    ...
    if best_state is not None:
        ...
        try:
            ...
            if not isinstance(payload, Mapping) or payload.get("role") != CHECKPOINT_ROLE_V3_A2:
                _fail("V3-A2 checkpoint role changed during reload")
            reload_base = build_meter_refiner(FROZEN_D11_CONFIG)
            reload_base.load_state_dict(dict(audited.model_state), strict=True)
            reloaded = build_meter_classification_adapter_v3_a2(reload_base)
            reloaded.load_state_dict(payload["model_state_dict"], strict=True)
            assert_model_finite(reloaded)
        except MeterRealDomainAdaptationV3A2Error:
            raise
        except Exception as exc:
            raise MeterRealDomainAdaptationV3A2Error("V3-A2 checkpoint cannot be strictly reloaded") from exc
        if model_state_sha256(reloaded) != candidate_state_sha or model_state_sha256(reloaded.base) != base_state_sha:
            _fail("V3-A2 checkpoint state hash mismatch after reload")
        checkpoint_reload_verified = True

    metrics = {
        "schema_version": METRICS_SCHEMA_V3_A2,
        "adaptation_version": METER_REAL_DOMAIN_ADAPTATION_V3_A2,
        "status": status,
        "run_id": run_id,
        "repository_sha": repository_sha,
        ...
        "real_positive_margin": {
            "scope": "real-train-positive-only",
            "loss_weight_milli": config.positive_margin_loss_milli,
            "margin_milli": config.positive_margin_milli,
            "validation_record_special_cases": 0,
        },
    ...
    verification = {
        "schema_version": VERIFICATION_SCHEMA_V3_A2,
        "adaptation_version": METER_REAL_DOMAIN_ADAPTATION_V3_A2,
        "status": status,
        "run_id": run_id,
        "repository_sha": repository_sha,
        ...
        "d11_fully_frozen": True,
        "bbox_frozen_exact": True,
        "real_positive_margin_train_only": True,
        "validation_record_special_cases": 0,
