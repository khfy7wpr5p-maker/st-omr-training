#!/usr/bin/env python3
"""Durable Colab background runner for Meter V3-A1.

Reuses the verified Teacher Gold and D10 cache boundaries while running the
classification-only, exact-bbox-frozen V3-A1 shadow experiment.
"""
...
def _run(args: argparse.Namespace, status: DurableStatus) -> dict[str, object]:
    ...
    from st_omr_training.meter_real_domain_adaptation_v3_a1_run import (
        run_meter_real_domain_adaptation_v3_a1,
    )
    ...
    status.update(
        "teacher_gold_started",
        {
            ...
            "phase_index": 1,
            "phase_total": 9,
            "epochs_total": FROZEN_ADAPTATION_CONFIG_V3_A1.epochs,
        },
    )
    ...
    def progress(event: str, payload: Mapping[str, object]) -> None:
        ...
        translated.setdefault("epochs_total", FROZEN_ADAPTATION_CONFIG_V3_A1.epochs)
        status.update(event, translated)

    return run_meter_real_domain_adaptation_v3_a1(
