#!/usr/bin/env python3
"""Durable Colab background runner for Meter real-domain retention V3.

This runner reuses the verified V1/V2 cache and Teacher Gold boundaries.  The
only training change is delegated to ``run_meter_real_domain_retention_v3``:
the frozen midpoint learning-rate schedule.  Existing V2 run artifacts are
never overwritten.
"""
...
def _run(args: argparse.Namespace, status: DurableStatus) -> dict[str, object]:
    ...
    status.update(
        "teacher_gold_started",
        {
            ...
            "phase_index": 1,
            "phase_total": 9,
            "epochs_total": TOTAL_EPOCHS_V3,
            "retention_v3_schedule": schedule,
        },
    )
    ...
    status.update(
        ...
        {"d11_checkpoint_sha256": observed_d11, "retention_v3_schedule": schedule},
    )
    ...
    def progress(event: str, payload: Mapping[str, object]) -> None:
        ...
        translated.setdefault("epochs_total", TOTAL_EPOCHS_V3)
        translated.setdefault("retention_v3_schedule", schedule)
        status.update(event, translated)

    return run_meter_real_domain_retention_v3(
    ...
    try:
        ...
        status.finish(
            ...
            payload={
                ...
                "retention_version": metrics["retention_version"],
                "retention_schedule": metrics["retention_schedule"],
                "retention_receipt_sha256": metrics["retention_receipt_sha256"],
